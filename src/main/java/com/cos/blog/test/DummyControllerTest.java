...
//		userRepository.save(user);
	...
	public User detail(@PathVariable int id) {
		// user/4를 찾으면 내가 테이터베이스에서 못찾아 오게 되면 user가 null이 될 거 아냐?
		// 그럼 return null이 리턴이 되잖아.. 그럼 프로그램에 문제가 있지 않겠니?
		// Optional로 너의 User객체를 감싸서 가져올테니 null인지 아닌지 판단해서 return해!!

		// 람다식
		// User user = userRepository.findById(id).orElseThrow(()->{
		// return new IlegalArgumentException("해당 사용자는 없습니다.");
		// });
		...
		// 스플링부트 = MessageConverter라는 애가 응답시에 자동 작동
